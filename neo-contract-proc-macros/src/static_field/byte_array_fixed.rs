...
/// Public interface for the byte_array_fixed attribute macro
pub fn byte_array_fixed(attr: TokenStream, item: TokenStream) -> TokenStream {
    generate(attr, item)
}

/// Process the byte_array attribute macro
pub fn generate(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    // Extract the byte array value from the attribute arguments
    if args.len() != 1 {
        return syn::Error::new_spanned(
            &args[0],
            "Expected exactly one argument for byte_array attribute"
        )
        .to_compile_error()
        .into();
    }
    ...
    let hex_string = match &args[0] {
        ...
        _ => {
            return syn::Error::new_spanned(
                &args[0],
                "Expected string literal for byte array"
            )
            .to_compile_error()
            .into();
        }
    ...
    // Generate the output with the byte array initialization
