...
impl ReentrancyGuard {
    ...
    pub fn new(identifier: &str) -> Self {
        ...
        Self { key, status: ReentrancyStatus::NotEntered }
    }
    ...
    pub fn exit(&mut self) { self.status = ReentrancyStatus::NotEntered; }

    /// Check if the guard is currently entered
    pub fn is_entered(&self) -> bool { self.status == ReentrancyStatus::Entered }
}
