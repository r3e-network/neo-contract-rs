    ...
    match op {
        ...
        Operator::Call { function_index } => {
            ...
            if let Some(offset) = function_offsets.get(&func_idx) {
                ...
                return Err(Error::invalid_wasm(format!(
                    "Function index {} not found in function offsets",
                    func_idx
                )));
            }
        }
        ...
        Operator::I32Load { memarg } => {
            ...
            let offset = u32::try_from(memarg.offset).unwrap_or_else(|_| {
                panic!("Memory offset {} is too large for Neo VM", memarg.offset)
            });
        ...
        _ => {
            return Err(Error::UnsupportedWasmFeature(format!(
                "Unsupported WASM instruction: {:?}",
                op
            )));
        }
    }
