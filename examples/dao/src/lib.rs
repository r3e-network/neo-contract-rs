...
mod dao_contract {
    ...
    impl Codec for Proposal {
        ...
        fn decode(bytes: &[u8]) -> Result<Self> {
            ...
            let votes_for = u32::from_be_bytes([
                bytes[offset], bytes[offset+1], bytes[offset+2], bytes[offset+3]
            ]);
            offset += 4;
            
            let votes_against = u32::from_be_bytes([
                bytes[offset], bytes[offset+1], bytes[offset+2], bytes[offset+3]
            ]);
            offset += 4;
    ...
    impl DaoContract {
        ...
        fn new(voting_period_hours: u32, min_votes_to_pass: u32) -> Self {
            ...
            let owner = Runtime::check_witness()
        ...
        fn update_proposal_state(&mut self, proposal_id: u32) -> Result<()> {
            ...
            if self.is_proposal_ended(&proposal) {
                ...
                let min_votes = self.min_votes_to_pass.get()
                    .unwrap_or(None)
                    .unwrap_or(1);
                
                // Determine new state based on votes
                if proposal.votes_for > proposal.votes_against && 
                   proposal.votes_for >= min_votes {
        ...
        fn add_member(&mut self, address: H160) -> bool {
            ...
            Runtime::notify(&ByteString::from("MemberAdded"), &event_data);
        ...
        fn create_proposal(&mut self, title: String, description: String) -> u32 {
            ...
            // Emit event
            let mut event_data = Array::new();
            event_data.push(Any::from(new_id));
            event_data.push(Any::from(caller));
            event_data.push(Any::from(title));
            event_data.push(Any::from(end_time));
            Runtime::notify(&ByteString::from("ProposalCreated"), &event_data);
        ...
        fn vote(&mut self, proposal_id: u32, vote_for: bool) -> bool {
            ...
            event_data.push(Any::from(caller));
            event_data.push(Any::from(vote_for));
            Runtime::notify(&ByteString::from("VoteCast"), &event_data);
        ...
        fn execute_proposal(&mut self, proposal_id: u32) -> bool {
            ...
            event_data.push(Any::from(proposal_id));
            event_data.push(Any::from(caller));
            Runtime::notify(&ByteString::from("ProposalExecuted"), &event_data);
        ...
        fn remove_member(&mut self, address: H160) -> bool {
            ...
            let mut event_data = Array::new();
            event_data.push(Any::from(address));
            Runtime::notify(&ByteString::from("MemberRemoved"), &event_data);
        ...
        fn transfer_ownership(&mut self, new_owner: H160) -> bool {
            ...
            Runtime::notify(&ByteString::from("OwnershipTransferred"), &event_data);
        ...
        #[safe]
        fn is_member(&self, address: H160) -> bool {
        ...
        #[safe]
        fn get_proposal(&self, proposal_id: u32) -> Option<Proposal> {
        ...
        #[safe]
        fn get_vote(&self, proposal_id: u32, voter: H160) -> Option<Vote> {
            let vote_key = self.create_vote_key(proposal_id, &voter);
            self.votes.get(&vote_key)
                .unwrap_or(None)
        }
        ...
        fn get_voting_period(&self) -> u32 {
            let seconds = self.voting_period.get()
                .unwrap_or(None)
                .unwrap_or(86400);
        ...
        #[safe]
        fn get_min_votes(&self) -> u32 {
            self.min_votes_to_pass.get()
                .unwrap_or(None)
                .unwrap_or(1)
        }
    }
