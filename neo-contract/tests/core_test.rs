...
mod mock_events {
    ...
    pub fn get_events() -> Vec<(ByteString, Array<Any>)> { MOCK_EVENTS.with(|events| events.borrow().clone()) }
}
...
fn create_h160_from_bytes(bytes: [u8; 20]) -> H160 { H160::from_slice(&bytes) }
...
mod tests {
    ...
    impl TransferEvent {
        fn new(from: H160, to: H160, amount: u64) -> Self { Self { from, to, amount } }
