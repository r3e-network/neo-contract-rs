...
fn mock_nep17_transfer(args: Vec<Vec<u8>>) -> Result<Vec<u8>, String> {
    ...
    // Emit transfer event
    MockRuntime::notify("Transfer", vec![from.clone(), to.clone(), amount.to_le_bytes().to_vec()]);
...
mod tests {
    ...
    fn get_balance(account: &[u8]) -> u64 {
        let key = [b"balance:".to_vec(), account.to_vec()].concat();
        MockStorage::get(&key).map(|v| u64::from_le_bytes(v.try_into().unwrap_or([0; 8]))).unwrap_or(0)
    }
    ...
    fn test_transfer_success() {
        ...
        let result = fixture.run(|| {
            let args = vec![account1.clone(), account2.clone(), 200u64.to_le_bytes().to_vec()];
