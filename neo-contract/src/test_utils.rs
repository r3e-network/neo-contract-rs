...
impl MockStorage {
    ...
    pub fn new() -> Self { Self { data: BTreeMap::new(), events: Vec::new() } }
...
impl MockRuntime {
    ...
    pub fn mock_check_witness(_address: &H160) -> bool { true }
    ...
    pub fn mock_storage_get(key: &[u8]) -> Vec<u8> { MockStorage::get(key).unwrap_or_default() }

    /// Mock storage put operation
    pub fn mock_storage_put(key: &[u8], value: &[u8]) { MockStorage::put(key, value); }

    /// Mock storage delete operation
    pub fn mock_storage_delete(key: &[u8]) { MockStorage::delete(key); }
}
