...
impl ContractSimulator {
        ...
        E: Into<String>,
    {
        ...
        let initial_storage = if self.capture_storage_changes { MockStorage::get_all() } else { Vec::new() };
        ...
        // Get debug events
        let debug_events = if self.capture_debug { DebugCapture::events() } else { Vec::new() };
    ...
    fn calculate_storage_changes(
        ...
        for (key, value) in final_state {
            if let Some(old_value) = initial_map.get(key) {
                ...
                changes.push(StorageChange::Put { key: key.clone(), value: value.clone(), old_value: None });
            }
        ...
        for (key, value) in initial_map {
            changes.push(StorageChange::Delete { key: (*key).clone(), old_value: Some((*value).clone()) });
        }
    ...
    /// Gets debug report
    pub fn debug_report(&self) -> String { DebugCapture::generate_report() }
}
