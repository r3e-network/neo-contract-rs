...
impl CallFlags {
    ...
    pub fn new(value: u8) -> Self { CallFlags(value) }

    /// Gets the raw value of the flags
    pub fn bits(&self) -> u8 { self.0 }

    /// Checks if the flags contain the given flag
    pub fn contains(&self, flag: CallFlags) -> bool { (self.0 & flag.0) == flag.0 }

    /// Adds a flag
    pub fn add(&mut self, flag: CallFlags) { self.0 |= flag.0; }

    /// Removes a flag
    pub fn remove(&mut self, flag: CallFlags) { self.0 &= !flag.0; }

    /// Creates a new CallFlags with the given flag added
    pub fn with(self, flag: CallFlags) -> Self { CallFlags(self.0 | flag.0) }

    /// Creates a new CallFlags with the given flag removed
    pub fn without(self, flag: CallFlags) -> Self { CallFlags(self.0 & !flag.0) }

    /// Checks if the call is read-only
    pub fn is_read_only(&self) -> bool { self.contains(Self::READ_ONLY) }

    /// Checks if the call allows notifications
    pub fn allows_notify(&self) -> bool { self.contains(Self::ALLOW_NOTIFY) }

    /// Checks if the call allows state changes
    pub fn allows_states(&self) -> bool { self.contains(Self::ALLOW_STATES) }

    /// Checks if the call allows writing to storage
    pub fn allows_write(&self) -> bool { self.contains(Self::ALLOW_WRITE) }

    /// Checks if the call allows calling other contracts
    pub fn allows_call(&self) -> bool { self.contains(Self::ALLOW_CALL) }
}
...
impl From<u8> for CallFlags {
    fn from(value: u8) -> Self { CallFlags(value) }
}

impl From<CallFlags> for u8 {
    fn from(flags: CallFlags) -> Self { flags.0 }
}

impl core::ops::BitOr for CallFlags {
    type Output = Self;

    fn bitor(self, rhs: Self) -> Self::Output { CallFlags(self.0 | rhs.0) }
}

impl core::ops::BitAnd for CallFlags {
    type Output = Self;

    fn bitand(self, rhs: Self) -> Self::Output { CallFlags(self.0 & rhs.0) }
}

impl core::ops::BitXor for CallFlags {
    type Output = Self;

    fn bitxor(self, rhs: Self) -> Self::Output { CallFlags(self.0 ^ rhs.0) }
}
