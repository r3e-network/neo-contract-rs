    ...
    fn partial_cmp(&self, other: &Self) -> Option<core::cmp::Ordering> {
        ...
        for i in (0..32).rev() {
            if self.0[i] < other.0[i] {
                return Some(if self_negative { core::cmp::Ordering::Greater } else { core::cmp::Ordering::Less });
            }
            if self.0[i] > other.0[i] {
                return Some(if self_negative { core::cmp::Ordering::Less } else { core::cmp::Ordering::Greater });
            }
        }
...
impl Ord for Int256 {
    fn cmp(&self, other: &Self) -> core::cmp::Ordering { self.partial_cmp(other).unwrap() }
}

impl Int256 {
    /// Creates a new Int256 with value zero
    pub fn zero() -> Self { Int256([0; 32]) }
    ...
    /// Tries to convert the Int256 to a u64
    pub fn as_u64(&self) -> Option<u64> { self.to_u64() }
...
impl From<i8> for Int256 {
    fn from(val: i8) -> Self { Int256::from_i64(val as i64) }
}
...
impl From<i16> for Int256 {
    fn from(val: i16) -> Self { Int256::from_i64(val as i64) }
}
...
impl From<i32> for Int256 {
    fn from(val: i32) -> Self { Int256::from_i64(val as i64) }
}

impl From<u64> for Int256 {
    fn from(val: u64) -> Self { Int256::from_u64(val) }
}

impl From<i64> for Int256 {
    fn from(val: i64) -> Self { Int256::from_i64(val) }
}
