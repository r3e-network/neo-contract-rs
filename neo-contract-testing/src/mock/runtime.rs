...
impl MockRuntime {
    ...
    pub fn get_block_height() -> u32 { BLOCK_HEIGHT.with(|h| *h.borrow()) }

    /// Set block height
    pub fn set_block_height(height: u32) { BLOCK_HEIGHT.with(|h| *h.borrow_mut() = height); }

    /// Get current timestamp
    pub fn get_time() -> u64 { TIMESTAMP.with(|t| *t.borrow()) }

    /// Set timestamp
    pub fn set_time(timestamp: u64) { TIMESTAMP.with(|t| *t.borrow_mut() = timestamp); }

    /// Get gas left
    pub fn get_gas_left() -> i64 { GAS_LEFT.with(|g| *g.borrow()) }

    /// Set gas left
    pub fn set_gas_left(gas: i64) { GAS_LEFT.with(|g| *g.borrow_mut() = gas); }

    /// Consume gas
    pub fn consume_gas(gas: i64) { GAS_LEFT.with(|g| *g.borrow_mut() -= gas); }

    /// Check if a witness is valid
    pub fn check_witness(witness: &[u8]) -> bool { WITNESSES.with(|w| w.borrow().contains(&witness.to_vec())) }

    /// Add a witness
    pub fn add_witness(witness: &[u8]) { WITNESSES.with(|w| w.borrow_mut().insert(witness.to_vec())); }

    /// Remove a witness
    pub fn remove_witness(witness: &[u8]) { WITNESSES.with(|w| w.borrow_mut().remove(&witness.to_vec())); }

    /// Clear all witnesses
    pub fn clear_witnesses() { WITNESSES.with(|w| w.borrow_mut().clear()); }

    /// Get all witnesses
    pub fn get_all_witnesses() -> Vec<Vec<u8>> { WITNESSES.with(|w| w.borrow().iter().cloned().collect()) }

    /// Get attribute
    pub fn get_attribute(type_id: u8) -> Option<Vec<u8>> { ATTRIBUTES.with(|a| a.borrow().get(&type_id).cloned()) }
    ...
    /// Clear attribute
    pub fn clear_attribute(type_id: u8) { ATTRIBUTES.with(|a| a.borrow_mut().remove(&type_id)); }

    /// Get calling script hash
    pub fn get_calling_script_hash() -> Vec<u8> { CALLING_SCRIPT_HASH.with(|h| h.borrow().clone()) }

    /// Set calling script hash
    pub fn set_calling_script_hash(hash: &[u8]) { CALLING_SCRIPT_HASH.with(|h| *h.borrow_mut() = hash.to_vec()); }

    /// Get executing script hash
    pub fn get_executing_script_hash() -> Vec<u8> { EXECUTING_SCRIPT_HASH.with(|h| h.borrow().clone()) }

    /// Set executing script hash
    pub fn set_executing_script_hash(hash: &[u8]) { EXECUTING_SCRIPT_HASH.with(|h| *h.borrow_mut() = hash.to_vec()); }

    /// Get entry script hash
    pub fn get_entry_script_hash() -> Vec<u8> { ENTRY_SCRIPT_HASH.with(|h| h.borrow().clone()) }

    /// Set entry script hash
    pub fn set_entry_script_hash(hash: &[u8]) { ENTRY_SCRIPT_HASH.with(|h| *h.borrow_mut() = hash.to_vec()); }
    ...
    /// Set random seed
    pub fn set_random_seed(seed: u32) { RANDOM_SEED.with(|s| *s.borrow_mut() = seed); }
    ...
    pub fn notify<T: AsRef<[u8]>>(name: &str, args: Vec<T>) {
        let args_vec: Vec<Vec<u8>> = args.into_iter().map(|arg| arg.as_ref().to_vec()).collect();
    ...
    pub fn verify_notification(name: &str) -> bool { MockEvents::has_event(name) }

    /// Clear all notifications
    pub fn clear_notifications() { MockEvents::reset(); }
}
