    ...
    V: Codec,
{
    ...
    pub fn get(&self, key: &K) -> Result<Option<V>> {
        ...
        let value =
            if let Some(context) = &self.context { context.get(&storage_key) } else { super::get(&storage_key) };
    ...
    pub fn put(&self, key: &K, value: &V) -> Result<()> { self.set(key, value) }
    ...
    /// Removes a value from the map
    pub fn remove(&self, key: &K) -> Result<()> { self.delete(key) }
    ...
    pub fn iter(&self) -> Vec<(K, V)> { self.find(FindOptions::default()) }
