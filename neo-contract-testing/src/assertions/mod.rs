...
use crate::mock::{MockEvents, MockRuntime, MockStorage};
use crate::TestError;
...
pub fn assert_block_height(expected: u32) -> Result<(), TestError> {
    ...
    if actual == expected {
        ...
        Err(TestError::new(format!("Block height does not match. Expected {}, got {}", expected, actual)))
    }
}
...
pub fn assert_timestamp(expected: u64) -> Result<(), TestError> {
    ...
    if actual == expected {
        ...
        Err(TestError::new(format!("Timestamp does not match. Expected {}, got {}", expected, actual)))
    }
}
...
pub fn assert_event_count(name: &str, expected: usize) -> Result<(), TestError> {
    ...
    if actual == expected {
        ...
        Err(TestError::new(format!("Event count for {} does not match. Expected {}, got {}", name, expected, actual)))
    }
}
...
pub fn assert_total_event_count(expected: usize) -> Result<(), TestError> {
    ...
    if actual == expected {
        ...
        Err(TestError::new(format!("Total event count does not match. Expected {}, got {}", expected, actual)))
    }
}
...
pub fn assert_event_args(name: &str, args: &[&[u8]]) -> Result<(), TestError> {
    ...
    Err(TestError::new(format!("Event {} with specified arguments not found", name)))
}
...
pub fn assert_gas_consumed_le(expected: i64) -> Result<(), TestError> {
    ...
    if gas_consumed <= expected {
        ...
        Err(TestError::new(format!("Gas consumed ({}) is greater than expected ({})", gas_consumed, expected)))
    }
}
...
pub fn assert_storage_prefix_contains(prefix: &[u8], expected_pairs: &[(&[u8], &[u8])]) -> Result<(), TestError> {
    let items = MockStorage::get_pairs_with_prefix(prefix);
...
mod tests {
    ...
    fn test_storage_assertions() {
        ...
        let pairs = [(b"key1" as &[u8], b"value1" as &[u8]), (b"key2" as &[u8], b"value2" as &[u8])];
        assert!(assert_storage_contains(&pairs).is_ok());

        let wrong_pairs = [(b"key1" as &[u8], b"value1" as &[u8]), (b"key2" as &[u8], b"wrong" as &[u8])];
    ...
    fn test_storage_prefix_assertions() {
        ...
        let prefix1_pairs = [(b"key1" as &[u8], b"value1" as &[u8]), (b"key2" as &[u8], b"value2" as &[u8])];
