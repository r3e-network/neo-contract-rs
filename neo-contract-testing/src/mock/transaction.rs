...
impl MockTransaction {
    ...
    pub fn get_version() -> u8 { VERSION.with(|v| *v.borrow()) }

    /// Set transaction version
    pub fn set_version(version: u8) { VERSION.with(|v| *v.borrow_mut() = version); }

    /// Get transaction nonce
    pub fn get_nonce() -> u32 { NONCE.with(|n| *n.borrow()) }

    /// Set transaction nonce
    pub fn set_nonce(nonce: u32) { NONCE.with(|n| *n.borrow_mut() = nonce); }

    /// Get transaction sender
    pub fn get_sender() -> Vec<u8> { SENDER.with(|s| s.borrow().clone()) }

    /// Set transaction sender
    pub fn set_sender(sender: &[u8]) { SENDER.with(|s| *s.borrow_mut() = sender.to_vec()); }

    /// Get transaction system fee
    pub fn get_system_fee() -> i64 { SYSTEM_FEE.with(|f| *f.borrow()) }

    /// Set transaction system fee
    pub fn set_system_fee(fee: i64) { SYSTEM_FEE.with(|f| *f.borrow_mut() = fee); }

    /// Get transaction network fee
    pub fn get_network_fee() -> i64 { NETWORK_FEE.with(|f| *f.borrow()) }

    /// Set transaction network fee
    pub fn set_network_fee(fee: i64) { NETWORK_FEE.with(|f| *f.borrow_mut() = fee); }

    /// Get transaction valid until block
    pub fn get_valid_until_block() -> u32 { VALID_UNTIL_BLOCK.with(|b| *b.borrow()) }

    /// Set transaction valid until block
    pub fn set_valid_until_block(block: u32) { VALID_UNTIL_BLOCK.with(|b| *b.borrow_mut() = block); }

    /// Get transaction signers
    pub fn get_signers() -> Vec<Vec<u8>> { SIGNERS.with(|s| s.borrow().clone()) }

    /// Add a signer
    pub fn add_signer(signer: &[u8]) { SIGNERS.with(|s| s.borrow_mut().push(signer.to_vec())); }
    ...
    /// Clear all signers
    pub fn clear_signers() { SIGNERS.with(|s| s.borrow_mut().clear()); }

    /// Get transaction attribute
    pub fn get_attribute(type_id: u8) -> Option<Vec<u8>> { TX_ATTRIBUTES.with(|a| a.borrow().get(&type_id).cloned()) }
    ...
    pub fn clear_attribute(type_id: u8) { TX_ATTRIBUTES.with(|a| a.borrow_mut().remove(&type_id)); }

    /// Get all transaction attributes
    pub fn get_all_attributes() -> BTreeMap<u8, Vec<u8>> { TX_ATTRIBUTES.with(|a| a.borrow().clone()) }

    /// Clear all transaction attributes
    pub fn clear_all_attributes() { TX_ATTRIBUTES.with(|a| a.borrow_mut().clear()); }
}
