...
impl<T> Array<T> {
    ...
    pub fn new() -> Self { Array(Vec::new(), core::marker::PhantomData) }

    /// Creates an Array with the given capacity
    pub fn with_capacity(capacity: usize) -> Self { Array(Vec::with_capacity(capacity), core::marker::PhantomData) }

    /// Creates an Array from a vector
    pub fn from_vec(vec: Vec<Any>) -> Self { Array(vec, core::marker::PhantomData) }
    ...
    pub fn push<IntoAny: Into<Any>>(&mut self, value: IntoAny) { self.0.push(value.into()); }

    /// Pops a value from the end of the Array
    pub fn pop(&mut self) -> Option<Any> { self.0.pop() }

    /// Gets a reference to a value by index
    pub fn get(&self, index: usize) -> Option<&Any> { self.0.get(index) }

    /// Gets a mutable reference to a value by index
    pub fn get_mut(&mut self, index: usize) -> Option<&mut Any> { self.0.get_mut(index) }
    ...
    pub fn remove(&mut self, index: usize) -> Any { self.0.remove(index) }

    /// Inserts a value at the given index
    pub fn insert<IntoAny: Into<Any>>(&mut self, index: usize, value: IntoAny) { self.0.insert(index, value.into()); }

    /// Clears the Array
    pub fn clear(&mut self) { self.0.clear(); }

    /// Returns an iterator over the Array
    pub fn iter(&self) -> impl Iterator<Item = &Any> { self.0.iter() }

    /// Returns a mutable iterator over the Array
    pub fn iter_mut(&mut self) -> impl Iterator<Item = &mut Any> { self.0.iter_mut() }
    ...
    pub fn as_vec(&self) -> &Vec<Any> { &self.0 }

    /// Converts the Array into a vector
    pub fn into_vec(self) -> Vec<Any> { self.0 }
}
...
impl<T> From<Vec<Any>> for Array<T> {
    fn from(vec: Vec<Any>) -> Self { Array(vec, core::marker::PhantomData) }
}
