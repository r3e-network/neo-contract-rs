...
pub fn compile<P: AsRef<std::path::Path>>(wasm_path: P, output_dir: P, name: Option<&str>) -> Result<(), error::Error> {
...
pub fn quick_compile<P: AsRef<std::path::Path>>(wasm_path: P) -> Result<(), error::Error> {
    ...
    let output_dir = wasm_path_buf.parent().map(PathBuf::from).unwrap_or_else(|| PathBuf::from("."));

    let compiler = Compiler::new();
    let contract_name = wasm_path_buf.file_stem().unwrap_or_default().to_string_lossy().to_string();
...
pub fn compile_with_options<P: AsRef<Path>>(
    ...
    Ok(CompilationResult { nef_path, manifest_path, contract_name })
}
