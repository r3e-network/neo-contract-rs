...
impl MethodInfo {
    ...
    pub fn add_attribute(&mut self, attr: MethodAttribute) { self.attributes.push(attr); }

    /// Checks if the method is marked as safe (read-only)
    /// In Neo N3, safe methods are represented with "safe": true in the manifest
    pub fn is_safe(&self) -> bool { self.attributes.iter().any(|attr| *attr == MethodAttribute::Safe) }

    /// Checks if the method requires owner-only access
    pub fn is_owner_only(&self) -> bool { self.attributes.iter().any(|attr| *attr == MethodAttribute::OwnerOnly) }
...
///
/// * `Result<(), Error>` - Result of the operation
pub fn emit_method<F>(script: &mut Script, method_info: &MethodInfo, body_generator: F) -> Result<(), Error>
where F: FnOnce(&mut Script) -> Result<(), Error> {
