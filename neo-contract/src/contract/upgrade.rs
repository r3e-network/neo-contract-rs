...
pub fn contract_exists(script_hash: &H160) -> bool { ContractManagement::has_contract(script_hash) }
...
impl Upgradeable for ContractUpgrade {
    ...
    fn upgrade(nef_file: ByteString, manifest: ByteString, data: Option<Any>) -> Result<()> {
        ...
        let success = ContractManagement::update(&previous_hash, nef_file.as_bytes(), &manifest, data);
    ...
    fn record_migration(previous_hash: H160, new_hash: H160) -> Result<()> {
        ...
        let mut history = if let Some(bytes) = crate::storage::get(history_key) { bytes.to_vec() } else { Vec::new() };
    ...
    fn get_migration_history() -> Vec<MigrationRecord> {
        ...
        if let Some(bytes) = crate::storage::get(history_key) {
            ...
            for i in 0..record_count {
                ...
                records.push(MigrationRecord { previous_hash, new_hash, timestamp, block_height });
            }
        }
...
impl UpgradeableContractBuilder {
    ...
    pub fn is_initialized() -> bool { ContractUpgrade::get_admin().is_some() }
}
...
macro_rules! initialize_upgradeable {
    ($admin:expr) => {
        ...
        pub fn is_initialized() -> bool { $crate::contract::upgrade::ContractUpgrade::get_admin().is_some() }
    };
}
