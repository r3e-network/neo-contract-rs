...
pub fn get_invocation_operation() -> ByteString { syscall::system_runtime_get_trigger().into() }

/// Get the arguments from the current invocation
pub fn get_invocation_args() -> Array { syscall::system_runtime_get_notification_args() }

/// Set the result of the current invocation
pub fn set_invocation_result(result: Any) { syscall::system_runtime_log(&result); }

/// Get the current blockchain timestamp
pub fn time() -> u64 { syscall::system_runtime_get_time() }

/// Get the script hash that called the current method
pub fn calling_script_hash() -> H160 { syscall::system_runtime_get_caller() }

/// Check if the given script hash has witnessed the current transaction
pub fn check_witness(hash: &H160) -> bool { syscall::system_runtime_check_witness(hash) }

/// Entry point for deployment
#[doc(hidden)]
#[no_mangle]
pub extern "C" fn __neo_deploy_entry() -> bool { true }
...
impl Runtime {
    /// Get the current blockchain timestamp
    pub fn time() -> u64 { time() }

    /// Get the script hash that called the current method
    pub fn calling_script_hash() -> H160 { calling_script_hash() }
    ...
    /// Check if the given script hash has witnessed the current transaction
    pub fn check_witness(hash: &H160) -> bool { check_witness(hash) }
