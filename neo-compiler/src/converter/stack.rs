...
impl StackEmulator {
    ...
    pub fn push(&mut self, value: String) { self.items.push(value); }

    /// Pops a value from the stack.
    pub fn pop(&mut self) -> Option<String> { self.items.pop() }

    /// Returns the current stack depth.
    pub fn depth(&self) -> usize { self.items.len() }

    /// Peeks at the top value on the stack without removing it.
    pub fn peek(&self) -> Option<&String> { self.items.last() }
}
...
impl StackAnalyzer {
    ...
    pub fn new() -> Self { Self { max_stack_size: 0, current_stack_size: 0 } }
    ...
    pub fn analyze_function(&self, function: &WasmFunction) -> Result<usize, Error> {
        ...
        let locals_count =
            reader.read_var_u32().map_err(|e| Error::invalid_wasm(format!("Failed to read locals count: {}", e)))?;

        // Skip locals
        for _ in 0..locals_count {
            let _ =
                reader.read_var_u32().map_err(|e| Error::invalid_wasm(format!("Failed to read local count: {}", e)))?;
            let _ = reader.read_u8().map_err(|e| Error::invalid_wasm(format!("Failed to read local type: {}", e)))?;
        }
        ...
        while !reader.eof() {
            let operator =
                reader.read_operator().map_err(|e| Error::invalid_wasm(format!("Failed to read operator: {}", e)))?;
    ...
    fn analyze_operator(&mut self, op: Operator) -> Result<(), Error> {
        ...
        match op {
            ...
            Operator::Drop | Operator::LocalSet { .. } | Operator::GlobalSet { .. } => {
                self.current_stack_size -= 1;
            }
            ...
            // Control flow
            Operator::Block { .. } | Operator::Loop { .. } | Operator::If { .. } | Operator::Else | Operator::End => {
            ...
            Operator::I32WrapI64 | Operator::I64ExtendI32S => {
                // No change
            }
