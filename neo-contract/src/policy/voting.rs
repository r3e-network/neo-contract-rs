...
use crate::prelude::{Any, Array, ByteString, StorageMap, H160};
...
impl Codec for ProposalAction {
    ...
    fn decode(bytes: &[u8]) -> Result<Self> {
        ...
        Ok(ProposalAction { contract_hash, method, args })
    }
}
...
impl VotingSystem {
    ...
    pub fn new(prefix: &[u8], voting_token: Option<H160>) -> Self {
        Self { prefix: ByteString::from_bytes(prefix), voting_token }
    }
    ...
    pub fn check_proposal(&self, proposal_id: &str) -> Result<()> {
        ...
        proposal.status = if passed { ProposalStatus::Accepted } else { ProposalStatus::Rejected };
    ...
    pub fn execute_proposal(&self, proposal_id: &str) -> Result<()> {
        ...
        for action in &proposal.actions {
            let result =
                Runtime::call_contract(action.contract_hash.clone(), action.method.clone(), action.args.clone());
    ...
    fn get_voting_power(&self, address: &H160) -> u64 {
        if let Some(token_hash) = self.voting_token {
            ...
            let result = Runtime::call_contract(token_hash, method, args);
    ...
    fn get_current_block_height(&self) -> u32 {
        ...
        let _result = Runtime::call_contract(crate::contract::native::ledger::Ledger::hash(), method, args);

        // We don't need to use the block result directly, just get the index
        let height_method = ByteString::from("index");
        let height_result =
            Runtime::call_contract(crate::contract::native::ledger::Ledger::hash(), height_method, Array::new());
